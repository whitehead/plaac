...
plot_genes = function(infile="plot_data.txt", 
  ...
  plotWidth = 12;
  plotHeight = 3 + ifelse(showHMMProbs, 3, 0) + ifelse(showSeq, 4, 0)
  ...
  for (dex in sort(unique(datAll$ORDER))) {
    ...
    par(mfrow=c(1,1), family="Courier", mar=c(1,4,4,1)+0.1);
    plot_gene(dat, showHMMProbs=showHMMProbs, showParses=showParses, showSeq=showSeq,  
...
# 
# ############### Oct 2013
# plot_genes_old = function(infile="plot_data.txt", 
#                       outfile="plaac_plots.pdf",
#                       showHMMProbs=TRUE,
#                       showParses=FALSE, 
#                       showSeq=FALSE,  
