...
def find_jar(explicit: Optional[PathLike] = None) -> Path:
    ...
    raises rather than silently falling back to a different jar. Only when
    neither is given does it auto-discover the jar's usual built and shipped
    ...
    discovered = [
        here.parent / "target" / "plaac.jar",              # cli/target/plaac.jar (built)
        here.parent.parent / "web" / "bin" / "plaac.jar",  # web/bin/plaac.jar (shipped)
    ]
    for c in discovered:
        if c.is_file():
            return c.resolve()
    ...
    raise PlaacError(
        "Could not find plaac.jar. Build it with cli/build_plaac.sh, or pass "
        "jar=... or set $PLAAC_JAR. Searched:\n  " + searched
    )
