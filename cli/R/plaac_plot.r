...
resolutionPng = 300  # default value
resolutionArg = match("-r", args)
if (!is.na(resolutionArg)) {
  resolutionPng = as.numeric(args[resolutionArg + 1])
  args = args[-c(resolutionArg, resolutionArg + 1)]
}

if (length(args) < 2) {
  cat("usage: Rscript plaac_plot.r plotdata.txt figname [-d|-c] [-r png_resolution]\n")
  ...
  cat("   The optional -r integer gives the PNG resolution in DPI (default", resolutionPng, ").\n")
  ...
  color_code_seqs(args[1], args[2], max_n=max_n, showLegend=T, showParses=T, resolutionPng=resolutionPng)
} else if (length(args)>2 && args[3]=="-d") {
  plot_seqs(args[1], args[2], showSeq=T, showHMMProbs=T, showParses=T, showAAColors=T, resolutionPng=resolutionPng,
